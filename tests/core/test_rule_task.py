...
async def rule_task(sample_rule_config, event_bus):
    """创建一个规则任务实例"""
    task = RuleTask(
        rule_config=sample_rule_config, task_id="test_task_id", event_bus=event_bus
    )
...
class TestRuleTask:
    ...
    async def test_state_management(self, rule_task):
        ...
        # 创建新任务测试兼容方法
        new_task = RuleTask(
            rule_config=rule_task.rule_config,
            task_id="compat_test",
            event_bus=rule_task._event_bus,
        )
    ...
    @pytest.mark.asyncio
    async def test_check_rule_condition(
        self, rule_task, observable_ctx, sample_rule_config: RuleConfig
    ):
        """测试规则条件检查 - 类方法"""
        # 测试类方法 check_rule_condition

        # 初始状态下条件不满足（因为上下文中没有相应的值）
        assert not RuleTask.check_condition(
            sample_rule_config.match_condition, observable_ctx
        )

        # 设置上下文使条件满足
        observable_ctx["test_key"] = "test_value"
        assert RuleTask.check_condition(
            sample_rule_config.match_condition, observable_ctx
        )
        ...
        # 没有条件的规则应该默认满足
        assert RuleTask.check_condition(
            no_condition_rule.match_condition, observable_ctx
        )
    ...
    def test_check_condition_static_method(self, observable_ctx):
        ...
        assert RuleTask.check_condition(
            "'simple_key' in ctx and ctx.get('simple_key') == 'simple_value'",
            observable_ctx,
            log_id,
        )
        assert not RuleTask.check_condition(
            "'simple_key' in ctx and ctx.get('simple_key') == 'wrong_value'",
            observable_ctx,
            log_id,
        )
        ...
        assert RuleTask.check_condition(
            "'num1' in ctx and 'num2' in ctx and ctx.get('num1') + ctx.get('num2') > 25",
            observable_ctx,
            log_id,
        )
        assert not RuleTask.check_condition(
        ...
        # 测试无效条件（语法错误）
        assert not RuleTask.check_condition(
            "this is invalid python code!!!", observable_ctx, log_id
        )

        # 测试异常情况（引用不存在的变量）
        assert not RuleTask.check_condition(
            "non_existent_var > 10", observable_ctx, log_id
        )
    ...
    async def test_handle_tool_result_direct_return(self, rule_task, observable_ctx):
        ...
        await rule_task._handle_tool_result(
            ctx=observable_ctx, dispatcher=mock_dispatcher, response_full="测试结果"
        )
    ...
    async def test_execute_tool_success(self, rule_task, observable_ctx):
        ...
        with patch("core.tools.base.ToolCenter.run_tool", side_effect=mock_run_tool):
            # 执行工具
            chunks = []
            async for chunk in rule_task.execute_tool(
                ctx=observable_ctx, dispatcher=mock_dispatcher
            ):
                chunks.append(chunk)
