...
import logging
import uuid
from typing import List, Dict, Set, AsyncGenerator, Optional, Any
...
class RuleDispatcher:
    def __init__(
        ...
        self._tasks: Dict[str, RuleTask] = {} # 通过ID存储所有任务
        self._active_task_executions: Set[asyncio.Task] = set() # 跟踪正在运行的asyncio任务
        self._output_queue: asyncio.Queue[str] = asyncio.Queue() # 用于DIRECT_RETURN输出

        self._event_bus.subscribe(event_type="ctx_changed", callback=self._handle_ctx_changed)
    ...
    async def _handle_ctx_changed(self, event_data: Dict[str, Any]) -> None:
        ...
        logger.debug(f"Ctx changed: key='{changed_key}', changed value: {changed_value}")
        for task in self._tasks.values():
            if task.is_completed() or task.is_executing():
                # 只检查尚未完成或正在处理的任务
                continue
            if task.rule_config.depend_ctx_key is not None and changed_key in task.rule_config.depend_ctx_key:
                logger.debug(f"Task {task.task_id} depends on {changed_key}. Evaluating.")
                self._check_and_schedule_task_if_needed(task)
            else:
                logger.debug(f"Task {task.task_id} does not depend on {changed_key}. Skipping.")

    def _check_and_schedule_task_if_needed(self, task: RuleTask) -> None:
        # 检查任务是否已经在执行中（避免重复调度）
        if task.is_executing():
            logger.debug(f"Task {task.task_id} is already executing. Skipping scheduling.")
            return

        if not task.is_condition_meet(self._ctx):
            logger.debug(f"Task {task.task_id} is not meet the condition. Skipping scheduling.")
            return

        logger.debug(f"Task {task.task_id} (Name: {task.rule_config.name if hasattr(task.rule_config, 'name') else 'N/A'}) is ready. Scheduling for execution.")
        task.set_executing(True) # 标记为执行中

        user_input_for_llm = self._ctx.get("user_input", "")
        logger.debug(f"User input for LLM: {user_input_for_llm}")

        execution_coro = task.execute(
            ctx=self._ctx,
            dispatcher=self,
            llm_client=self._llm_client,
            base_system_prompt=self._agent_system_prompt,
        ...
        async def task_wrapper():
            try:
                ...
                if not task.is_completed(): # 确保如果execute没有明确标记完成，则标记为完成
    ...
    async def get_output_stream(self) -> AsyncGenerator[str, None]:
        ...
        while True:
            ...
            if any_task_is_runnable:
                ...
                # （例如：上下文改变事件刚刚发生）
                # 等待事件循环处理
                logger.debug("get_output_stream: No active executions, but a task is ready. Yielding briefly.")
            ...
            logger.debug("No active executions, output queue was empty, and no task is currently runnable. Ending stream.")
            break

    async def shutdown(self) -> None:
        logger.debug("Shutting down RuleDispatcher...")
        ...
        logger.debug("All active task executions cancelled or finished.")
        # 清空输出队列
        while not self._output_queue.empty():
            self._output_queue.get_nowait()
            self._output_queue.task_done() # 如果使用task_done/join
        logger.debug("RuleDispatcher shut down.") 
