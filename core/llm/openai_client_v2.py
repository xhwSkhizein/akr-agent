...
# 假设 LLMClient 基类已定义，如果未定义，可以简单创建一个
...
class OpenAIClient(LLMClient):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        self.client = AsyncOpenAI(api_key=api_key, **kwargs.pop('client_args', {})) # 传递 client 的额外参数
        self.model = model
        ...
        while retry_count <= max_retries:
            try:
                ...
                async for chunk in response_stream:
                    ...
                    if delta and delta.tool_calls:
                        for tool_call_chunk in delta.tool_calls:
                            ...
                            if tool_id not in active_tool_calls_data:
                                active_tool_calls_data[tool_id] = {
                                    "id": tool_id,
                            ...
                            if tool_call_chunk.function:
                                if tool_call_chunk.function.name:
                                    active_tool_calls_data[tool_id]["function_name"] += tool_call_chunk.function.name
                                if tool_call_chunk.function.arguments:
                                    active_tool_calls_data[tool_id]["function_arguments"] += tool_call_chunk.function.arguments
                    
                    # 3. 当LLM指示工具调用完成时 (或者流自然结束)
                    if finish_reason == "tool_calls":
                        if not active_tool_calls_data:
                            logger.warning("finish_reason 是 'tool_calls' 但没有收集到工具调用数据。")
                            # 这种情况不应该发生，但以防万一
                            break 
                        ...
                        for tool_id, data in active_tool_calls_data.items():
                            if data["function_name"] and data["id"]: # 参数可能是空字符串，但名称和ID必须有
                                assistant_tool_calls_list.append(
                                    ChatCompletionMessageToolCall(
                                        ...
                                        function={
                                            ...
                                            "arguments": data["function_arguments"],
                                        }
                                    )
                                )
                                tools_to_execute_details.append(data)
                            else:
                                logger.warning(f"收集到的工具调用数据不完整，ID {tool_id}: {data}")

                        if not assistant_tool_calls_list:
                             logger.error("finish_reason='tool_calls' 但没有有效的工具调用可执行。")
                        ...
                        for tool_data in tools_to_execute_details:
                            tool_name = tool_data["function_name"]
                            tool_args = tool_data["function_arguments"]
                            tool_call_id = tool_data["id"]
                            
                            logger.info(f"开始调用工具: {tool_name}, 参数: {tool_args}, ID: {tool_call_id}")
                            try:
                                tool_result_content = await run_tool_func(tool_name, tool_args)
                                logger.info(f"工具 {tool_name} (ID: {tool_call_id}) 调用结果: {tool_result_content}")
                                tool_results_messages.append(
                                    {
                                        "role": "tool",
                                        "tool_call_id": tool_call_id,
                                        "name": tool_name, 
                                        "content": str(tool_result_content), # 结果必须是字符串
                                    }
                                )
                            except Exception as e:
                                logger.error(f"工具 {tool_name} (ID: {tool_call_id}) 执行失败: {e}", exc_info=True)
                                tool_results_messages.append(
                                    {
                                        "role": "tool",
                                        "tool_call_id": tool_call_id,
                ...
                if active_tool_calls_data and finish_reason != "tool_calls":
                    ...
                    logger.warning(f"流结束 (finish_reason: {finish_reason})，但仍有未处理的工具调用数据: {active_tool_calls_data}")
                ...
                if retry_count <= max_retries:
                    ...
                    logger.warning(f"API速率限制。将在 {wait_time}s 后重试。尝试 {retry_count}/{max_retries}")
                ...
                if retry_count <= max_retries:
                    wait_time = backoff_factor ** retry_count
                    logger.warning(f"连接错误: {e}. 将在 {wait_time}s 后重试。尝试 {retry_count}/{max_retries}")
                    await asyncio.sleep(wait_time)
                else:
    ...
    def _prepare_params(
        ...
        **kwargs,
        ...
        if "tool_choice" in kwargs: # e.g., "auto", "none", {"type": "function", "function": {"name": "my_function"}}
...
async def example_run_tool_func(tool_name: str, tool_args: str) -> Any:
    """示例工具执行函数"""
    logger.info(f"示例工具执行: name='{tool_name}', args='{tool_args}'")
    if tool_name == "get_current_weather":
        # 实际应用中，这里会解析 tool_args (通常是 JSON 字符串)
        # import json
        # args_dict = json.loads(tool_args)
        # location = args_dict.get("location")
        # unit = args_dict.get("unit", "celsius")
        # ... 调用天气API ...
        return f"天气在 '{tool_args}' 是晴朗的。" # 简化返回
    elif tool_name == "search_web":
        return f"搜索 '{tool_args}' 的结果是：..."
    else:
...
async def main_example():
    ...
    # 请替换为你的 API Key 或确保设置了 OPENAI_API_KEY 环境变量
    client = OpenAIClient(api_key="sk-YOUR_API_KEY_HERE_IF_NOT_USING_ENV_VAR") 
    
    system_p = "你是一个乐于助人的AI助手，可以使用工具来获取信息。"
    # user_q = "今天上海的天气怎么样？用摄氏度告诉我。"
    user_q = "帮我查一下'异步Python编程'这个概念，并告诉我北京今天的天气。"


    # 定义希望LLM使用的工具 (符合OpenAI规范)
    example_tools = [
        {
            "type": "function",
            "function": {
    ...
    try:
        async for chunk in client.invoke_stream(
            ...
            user_input=user_q,
            run_tool_func=example_run_tool_func, # 传递工具执行函数
            tools=example_tools,                 # 传递可用工具列表
            tool_choice="auto",                  # 让LLM自动选择是否使用工具及使用哪个
            # temperature=0.1 # 可以覆盖默认温度
        ):
            print(chunk, end="", flush=True)
            full_response.append(chunk)
        print("\n--- 流式响应结束 ---")
        logger.info(f"完整响应: {''.join(full_response)}")
    except Exception as e:
        logger.error(f"主示例中发生错误: {e}", exc_info=True)

if __name__ == "__main__":
    # 注意: 如果在Jupyter Notebook等环境中运行，asyncio.run() 可能需要特殊处理
    # 例如，如果已有事件循环，使用 await main_example()
    # 或者 nest_asyncio.apply()
    try:
        asyncio.run(main_example())
    except RuntimeError as e:
        if "cannot run event loop while another loop is running" in str(e) and "asyncio" in str(type(e)):
            print("检测到已运行的事件循环。如果您在Jupyter Notebook中，请直接 'await main_example()'。")
            # 如果你想在脚本中强制允许嵌套（通常不推荐，但对某些IDE/notebook有用）
            # import nest_asyncio
            # nest_asyncio.apply()
            # asyncio.run(main_example()) # 再次尝试
        else:
            raise